...
const createContactProspectusSchema = (tenantId) => {
    const contactProspectusSchema = new mongoose.Schema(
        {
            name: {
                type: String,
    ...
    contactProspectusSchema.pre(/^find/, function () {
        this.where({ 'deleted.isDeleted': false })
    })

    return contactProspectusSchema
}

const contactProspectusModel = (tenantId) => {
    if (tenantId == null || tenantId == '') {
        throw new Error('Tenant id must be a value')
    }

    const collectionName = `${tenantId}_contactProspectuses`

    if (mongoose.models[collectionName]) {
        return mongoose.models[collectionName]
    }

    const schema = createContactProspectusSchema(tenantId)
    return mongoose.model(collectionName, schema)
}

module.exports = contactProspectusModel

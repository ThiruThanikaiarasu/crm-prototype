...
const createCompanyProspectusSchema = (tenantId) => {
    const companyProspectusSchema = new mongoose.Schema(
        {
            name: {
                type: String,
                required: [true, 'Company name is a mandatory field'],
            ...
            phone: {
                ...
                number: {
                    type: String,
                    required: [true, 'Company phone number is a mandatory field'],
    ...
    companyProspectusSchema.index(
        { name: 1, 'deleted.isDeleted': 1 },
        { collation: { locale: 'ar', strength: 1 } }
    )

    companyProspectusSchema.pre(/^find/, function () {
        this.where({ 'deleted.isDeleted': false })
    })

    return companyProspectusSchema
}

const companyProspectusModel = (tenantId) => {
    if (tenantId == null || tenantId == '') {
        throw new Error('Tenant id must be a value')
    }

    const collectionName = `${tenantId}_companyProspectuses`

    if (mongoose.models[collectionName]) {
        return mongoose.models[collectionName]
    }

    const schema = createCompanyProspectusSchema(tenantId)
    return mongoose.model(collectionName, schema)
}

module.exports = companyProspectusModel
